...
class Trampoline {
    ...
    PtrKid removeFromList(std::string name, std::list<PtrKid>& lista) {
        for (auto it = lista.begin(); it != lista.end(); ++it) {
            if ((*it)->getName() == name) {
                PtrKid removed = *it; // Salva o elemento antes de remover
                lista.erase(it);
                return removed;
            }
        }
        return nullptr; // Retorna nullptr caso o nome não seja encontrado
    }
...
public:
    ...
    PtrKid removeKid(std::string name) {
        PtrKid removed = removeFromList(name, waiting);
        if (!removed) {
            removed = removeFromList(name, playing);
        }
        if (!removed) {
            std::cout << "fail: " << name << " nao esta no pula-pula\n";
        }
        return removed;
    }
